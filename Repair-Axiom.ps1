# =============================================================================
# AXIOM ARCHITECT: PHASE 4 RECOVERY (FINAL GOLDEN MASTER)
# =============================================================================
$ErrorActionPreference = 'Continue' # Prevent crash on Docker warnings
$RootPath = Get-Location

Write-Host ">>> [1/5] RELEASING SYSTEM LOCKS..." -ForegroundColor Yellow
# Kill lingering Node processes to unlock node_modules folder
taskkill /F /IM node.exe /T 2>$null
taskkill /F /IM next.exe /T 2>$null

Write-Host ">>> [2/5] SHUTTING DOWN DOCKER..." -ForegroundColor Yellow
docker compose down --volumes --remove-orphans 2>$null

Write-Host ">>> [3/5] PURGING CORRUPTED ARTIFACTS..." -ForegroundColor Red
# Force remove folders. If it fails, wait 1 sec and retry (Windows file system race condition fix)
$Artifacts = @("src", "public", ".next", "node_modules", "package.json", "package-lock.json", "tsconfig.json", "tailwind.config.ts", "postcss.config.js", "next.config.js", "next.config.mjs", "Dockerfile", "docker-compose.yml")
foreach ($item in $Artifacts) {
    if (Test-Path ($target = Join-Path $RootPath $item)) {
        Remove-Item -Path $target -Recurse -Force -ErrorAction SilentlyContinue
    }
}

Write-Host ">>> [4/5] GENERATING IMMUTABLE ARCHITECTURE..." -ForegroundColor Green
New-Item -Path src/app, src/components, public -ItemType Directory -Force | Out-Null

# --- DEPENDENCIES ---
$Pkg = @{
    name="axiom"; version="4.0.0"; private=$true
...
# --- CONFIGS (CRITICAL: STANDALONE MODE) ---
[IO.File]::WriteAllText((Join-Path $RootPath 'next.config.mjs'), '/** @type {import("next").NextConfig} */ const nextConfig = { output: "standalone" }; export default nextConfig;', [Text.Encoding]::UTF8)
...
export default function AxiomMap() {
    return (
        <MapContainer center={[51.0447, -114.0719]} zoom={11} className="h-full w-full rounded-lg">
...
const AxiomMap = dynamic(() => import("@/components/AxiomMap"), { ssr: false, loading: () => <p>Initializing Geo-Link...</p> });
...
# --- DOCKER SETUP (NO VOLUMES) ---
Write-Host ">>> [5/5] CONFIGURING DOCKER..." -ForegroundColor Green
