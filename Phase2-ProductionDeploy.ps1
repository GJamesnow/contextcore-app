# Phase2-ProductionDeploy.ps1
# Axiom Architect: Deploys the "Progressive Disclosure" Analysis Engine

$ErrorActionPreference = "Stop"
Write-Host ">>> AXIOM: DEPLOYING PRODUCTION ENGINE..." -ForegroundColor Cyan

# 1. CLEANUP (Remove the literal sandbox if created)
if (Test-Path "app/sandbox") {
    Remove-Item "app/sandbox" -Recurse -Force
    Write-Host ">>> Cleanup: Removed literal sandbox directory." -ForegroundColor Gray
}

# 2. DIRECTORY CHECK
$dirs = @("app/actions", "components/inputs")
foreach ($dir in $dirs) {
...
# 3. HIGHER POWER: SERVER ACTION (create-analysis.ts)
# Confirmed Secure via Gauntlet Review
$actionPath = "app/actions/create-analysis.ts"
$actionContent = "
'use server';
...
export async function createAnalysis(formData: FormData) {
  ...
  redirect(\`/analysis/\${newDeal.id}\`);
}
"
[System.IO.File]::WriteAllText($actionPath, $actionContent.Trim())
Write-Host ">>> Deployed Server Action: $actionPath" -ForegroundColor Green

# 4. UX LAYER: PROGRESSIVE DISCLOSURE HOME PAGE (app/page.tsx)
# Verified for Animation Smoothness and Maps Loading
...
"
[System.IO.File]::WriteAllText($homePath, $homeContent.Trim())
Write-Host ">>> Deployed UX Engine: $homePath" -ForegroundColor Green
Write-Host ">>> PHASE 2 COMPLETE. SYSTEM IS LIVE." -ForegroundColor Cyan
